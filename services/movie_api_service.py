# services/movie_api_service.py
from logger_setup import logger
from services._base_api_service import _BaseApiService, ITEMS_PER_PAGE
from services.recommendation_config import PERIODS, CULT_MOVIE_IDS
...
class MovieApiService(_BaseApiService):
    ...
    def fetch(self, period, genre_id=None, page=1, is_turkish=False) -> list:
        ...
        try:
            ...
            logger.warning(f"Film API rate limit aşıldı: {e.url}")
            return []
        except ApiError as e:
            logger.error(f"Film API hatası: {e}")
            return []
        except Exception as e:
            logger.error(f"Film API beklenmeyen hata: {e}")
            return []
    ...
    def _by_date(self, period, genre_id=None, page=1, is_turkish=False) -> list:
        ...
        return self._fetch_paginated(f"{_TMDB_BASE}/discover/movie", params, self._parse_item)
    ...
    def _top_rated(self, genre_id=None, page=1, is_turkish=False) -> list:
        if is_turkish:
            ...
            url = f"{_TMDB_BASE}/movie/top_rated"
    ...
    def _cult(self, page=1) -> list:
        ...
        movie_ids = CULT_MOVIE_IDS[start_idx:start_idx + ITEMS_PER_PAGE]
        results = []
    ...
    def _upcoming(self, genre_id=None, page=1, is_turkish=False) -> list:
        if is_turkish:
            ...
            url = f"{_TMDB_BASE}/movie/upcoming"
            params = {'api_key': self._get_key(KEYRING_KEY_TMDB), 'language': 'tr-TR', 'region': 'TR', 'page': page}
        return self._fetch_paginated(url, params, self._parse_item, genre_id=genre_id)
