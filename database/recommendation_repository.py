...
# get_connection kaldırıldı: bağlantı sızıntısı riskine karşı get_db kullanılır
            ...
            with get_db() as conn:
                for item in recommendations:
                    conn.execute('''
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (
                        category, period, genre_key, 1 if is_turkish else 0,
            ...
            with get_db() as conn:
                rows = conn.execute('''
                    ...
                    WHERE category = ? AND period = ? AND genre = ?
                          ...
                          AND fetched_at > ?
                    ORDER BY id ASC
                ''', (category, period, genre_key, 1 if is_turkish else 0, page, expiry_date)).fetchall()
            results = [CachedRecommendation.from_row(row).to_display_dict() for row in rows]
    ...
    def get_max_cached_page(self, category: str, period: str,
        ...
        try:
            with get_db() as conn:
                ...
                ''', (category, period, genre_key, 1 if is_turkish else 0, expiry_date)).fetchone()
            ...
            with get_db() as conn:
                count = conn.execute('''
                    ...
                    WHERE category = ? AND period = ? AND genre = ?
                          AND is_turkish = ? AND page = ?
                          AND fetched_at > ?
                ''', (category, period, genre_key, 1 if is_turkish else 0, page, expiry_date)).fetchone()[0]
            return count > 0
        except Exception as e:
        ...
        try:
            with get_db() as conn:
                deleted_count = conn.execute(
                    'DELETE FROM recommendation_cache WHERE fetched_at < ?', (expiry_date,)
                ).rowcount
            if deleted_count > 0:
